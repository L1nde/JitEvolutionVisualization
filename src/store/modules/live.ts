...
export default {
  ...
  mutations: {
    ...
    changeProject(state: State, body: {projectId: string, app: AppDetailDto}) {
      state.projectId = body.projectId;
      state.app = body.app;
    },
  },
  actions: {
    async focus({ commit, state }, body: {projectId: string, fileUri: string}) {
        if(body.projectId === state.projectId){
          commit("focus", body.fileUri);
        }
      },
      async changeProject({ commit, dispatch }, body: {projectId: string}) {
        commit("changeProject", {projectId: body.projectId, app: await API.app.get(body.projectId)});
        commit("focus", null);
      },
  }
